...
bot.command :ping do |event|
  ...
  m.edit "🏓 Pong! Upłynęło #{Time.now - event.timestamp}s."
end
bot.command :prefix do |event|
  event.respond "Mój prefix to `#{event.bot.prefix}`"
end
...
**Komendy 4FUN:**
  ...
  czy - zadaj pytanie, bot ci odpowie
**Smocze komendy:**
  wiki - daje ci link do smoczej wiki
...
bot.command :latlog do |event|
  event.respond ("**v0.6.1** \n Ulepszenia komend, wycofanie komendy bany, wycofanie komendy prefix (bo była trochę bez sensu), dodanie komendy czy,server, wiki")
end
...
bot.command :czy do |event|
  odp = ["`Tak`", "`Nie`", "`Oczywiście!`", "`NIEEEEE!`", "`Pewnie że tak!`", "`Daj mi spokój!`"].sample
end
#komendy moderacyjne
bot.command :user do |event|
  event.respond "**Informacje o użytkowniku** \n 🔰 Nazwa: `#{event.user.name}` \n 🌐 Tag: `##{event.user.tag}` \n ✅ ID: `#{event.user.id}` \n  🧧 Awatar: `#{event.user.avatar_url}`"
end
bot.command :server do |event|
  event.respond "**INFORMACJE O SERWERZE** \n 🔰 Nazwa: `#{event.server.name}`  \n 🛂 Członkowie: `#{event.server.member_count}` \n ⚠ Właściciel: `#{event.server.owner.name}##{event.server.owner.tag}` \n 🌐 ID: `#{event.server.id}` \n 📜 Region: `#{event.server.region}` \n ✨ Awatar: `#{event.server.icon_url}`"
end
#smocze komendy
bot.command :wiki do |event|
  event.respond "https://jakwytresowacsmoka.fandom.com/wiki/Jak_Wytresować_Smoka_Wiki"
end
...
bot.command :stop do |event|
  break unless event.user.id == 763847781176836126

  bot.send_message(event.channel.id, "Wyłączanie bota!")
  exit
end
#end
bot.ready() do |event|
